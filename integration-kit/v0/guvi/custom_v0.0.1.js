(function (win, doc) {
    ...
    win.ChateleonCustom = (function () {
        ...
        const custom = function (configs) {
            var MASCOT_CONTAINER_ID = configs.id ? ["chateleon-container", configs.id].join("-") : "chateleon-container";

            const addGuviClickListener = () => {
                var chateleonContainer = document.getElementById(MASCOT_CONTAINER_ID);
                ...
                for (const mutation of mutationList) {
                    if (mutation.type === "childList") {
                        ...
                        if (mutation.addedNodes[0]?.id == 'ymPluginDivContainerInitial') {
                            ...
                            const mascotCallback = function (container, mascot) {
                                // mascot.unsubscribe();
                            };

                            configs.isCustom = false;
                            configs.customImpl = customImpl;

                            const chateleon = new Chateleon(configs);
                            chateleon.subscribe(mascotCallback);

                            // Add mutation observer for chatBox
                            addChatBoxObserver();
                        }

                        if (mutation.addedNodes[0]?.id == MASCOT_CONTAINER_ID) {
                            console.log(MASCOT_CONTAINER_ID + ' container added');
