(function (win, doc) {
  ...
  const Chateleon = function (params) {
    ...
    let chameleonContainer = document.getElementById('chameleon-anim-image');
    let chateleonContainer = document.createElement('div');
    chateleonContainer.id = ['chateleon-container', params.id].join('-') ? ['chateleon-container', params.id].join('-') : 'chateleon-container';
    // chateleonContainer.className = 'justify-end w-2/4 left-2/3';
    ...
    var MASCOT_CONTAINER_ID = config.id ? ["chateleon-container", config.id].join("-") : "chateleon-container";
    var mascotContainer;
    let gif;
    ...
    const subscribe = function (callback) {
      if (config.isCustom) {
        console.warn("The method subscribe is not allowed here. Please use it inside the custom file.");
        return;
      }
              ...
              if (this.status === 200) {
                ...
                if (params.animationType === 'multiple') {
                  ...
                  for (let i = 0; i < params.meta.length; i++) {
                    ...
                    let MASCOT_CONTAINER_ID = params.meta[i].id ? params.meta[i].id : config.id;
                    let DURATION = params.meta[i].duration ? params.meta[i].duration : null;
                    ...
                    if (gifs.length) {
                      if (config.customImpl) {
                        config.customImpl(gifs[i], MASCOT_CONTAINER_ID, trackEvent);
                      } else {
                        animation(gifs, MASCOT, DURATION);
                      }
                      trackServe()
                      console.log(
                        'Subscribed the ' + `${MASCOT.id}` + " " + MASCOT.order + '!!!'
                      )
                    }
                  }
                  ...
                  if (gifBasedOnCurrentTime.length) {
                    if (config.customImpl) {
                      config.customImpl(gifBasedOnCurrentTime[0], MASCOT_CONTAINER_ID, trackEvent);
                    } else {
                      mascotContainer = createMascotContainer(gifs[0], MASCOT_CONTAINER_ID, config.iframeData);
                    }
    ...
    const filterGifOnCurrentTime = function (_gif) {
      ...
      if (_gif.active && (hour >= parseInt(startHourMin[0]) && hour <= parseInt(endHourMin[0])) && (minute >= parseInt(startHourMin[1]) && minute <= parseInt(endHourMin[1]))) {
        return _gif;
      }
    };
    ...
    const animation = function (gifs, MASCOT, DURATION) {
      ...
      const gifPromises = Gifs.map(gif => new Promise(resolve => {
        gif.onload = () => {
          resolve();
        };
      }));
    ...
    const animate = function (mascot, gifs, configs) {
      ...
      img.onload = () => {
        ...
        img.style.position = configs.position.type ? configs.position.type : 'absolute';
        img.style.height = mascot.dimensions.height ? mascot.dimensions.height : 100 + '%';
        img.style.width = mascot.dimensions.width ? mascot.dimensions.width : 100 + '%';
        img.style.zIndex = mascot.dimensions.zIndex ? mascot.dimensions.zIndex : 500;
        img.style.position = mascot.position.type ? mascot.position.type : 'absolute';
        img.style.bottom = mascot.position.bottom ? mascot.position.bottom : 0 + '%';
        img.style.right = mascot.position.right ? mascot.position.right : 0 + '%';

        // chateleonContainer.style.cssText = 'position:absolute;width:300px;z-index:100;bottom:300; right: 300;';
        chateleonContainer.style.position = configs.position.type ? configs.position.type : 'absolute';
        chateleonContainer.style.right = configs.position.right ? configs.position.right : 50 + '%';
        chateleonContainer.style.bottom = configs.position.bottom ? configs.position.bottom : 50 + '%';
        chateleonContainer.style.width = configs.dimensions.width ? configs.dimensions.width : 300 + 'px';
        chateleonContainer.style.height = configs.dimensions.height ? configs.dimensions.height : 180 + 'px';
        chateleonContainer.style.zIndex = mascot.dimensions.zIndex ? mascot.dimensions.zIndex : 500;

        // doc.body.appendChild(chateleonContainer)
        chameleonContainer.appendChild(chateleonContainer);
    ...
    const createMascotContainer = function (filteredGif, id, data) {
      ...
      if (data) {
        if (data.targetIframe) {
          ...
          var target = iframe.contentDocument.querySelector(data.targetContainer);
          if (data.prepend) {
            target.prepend(elem);
          } else {
            target.append(elem);
        ...
        elemDiv.style.cssText = 'position:absolute;width:300px;z-index:100;bottom:300; right: 300;';
    ...
    const moveGif = function (data, id) {
      ...
      data.visibility ? gifContainer.style.visibility = data.visibility : null;
    };
